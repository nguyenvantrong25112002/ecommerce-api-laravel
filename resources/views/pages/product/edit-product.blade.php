...
@section('content')
    <x-breadcrumb :datas="[
        ...
        ['active' => true, 'content' => 'Chỉnh sửa sản phẩm'],
        [
            'active' => false,
            'content' => $product->name,
            'route' => route('admin.product.edit.form', ['id' => $product->id]),
        ],
    ]" />
    @component('components.card-layout')
        @slot('conten')
            <form id="add_product" action="{{ route('admin.product.edit.save', ['id' => $product->id]) }}" method="POST"
                ...
                <div class="row">
                    <div class="col-9">
                        ...
                        <div class="form-group mb-5">
                            ...
                            <textarea class="form-control " name="description" id="kt_docs_ckeditor_classic" rows="2">
                                {{ $product->description }}
                            </textarea>
                        </div>
                        <div class="form-group mb-5">
                            <label for="">Chi tiết</label>
                            <textarea class="form-control " name="details" id="kt_docs_ckeditor_classic_2" rows="7">
                                {{ $product->details }}
                        ...
                        <div class="row mb-5">
                            <div class="col-3">
                                <x-select-status :data="$product->status"></x-select-status>
                            </div>
                            <div class="col-3">
                                @component('components.input-form',
                                    [
                    ...
                    <div class="col-3">
                            ...
                            <div class="overflow-auto h-250px">
                                @foreach ($categoryParent as $cateItemParent)
                                    ...
                                    <div class="form-check mt-3 py-1 ">
                                        <input
                                            @if (old('category_id')) @checked(collect( old('category_id'))->contains($cateItemParent->id))
                                            @else
                                                @checked(collect($category_id )->contains($cateItemParent->id)) @endif
                        ...
                        @component('components.input-preview-image',
                            ['lable' => 'Ảnh sản phẩm', 'name' => 'image', 'required' => true, 'value' => $product->image])
                        @endcomponent
